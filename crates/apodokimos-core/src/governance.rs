...
pub enum GovernanceActionType {
    ...
    RotateGovernance { new_threshold: u32, new_signers: Vec<GovernanceSigner> },
...
impl GovernanceConfig {
    ...
    pub fn validate(&self) -> Result<(), ApodokimosError> {
        ...
        if self.threshold <= n / 2 {
            return Err(ApodokimosError::Governance(
                format!("threshold {} must be > n/2 (={}) for Byzantine safety", 
                    self.threshold, n / 2)
            ));
        }
    ...
    pub fn new(config: GovernanceConfig) -> Result<Self, ApodokimosError> {
        ...
        for signer in &config.signers {
            ...
            let public_key = VerifyingKey::from_bytes(&key_bytes)
                .map_err(|e| ApodokimosError::Governance(
                    format!("invalid Ed25519 public key for {}: {:?}", signer.did, e)
                ))?;
            public_keys.insert(signer.did.clone(), public_key);
        }
        
        Ok(Self { config, public_keys })
    }
    ...
    pub fn verify_action(&self, action: &GovernanceAction) -> VerificationResult {
        ...
        for sig in &action.signatures {
            ...
            let public_key = match self.public_keys.get(&sig.signer_did) {
                Some(pk) => pk,
                None => return VerificationResult::UnauthorizedSigner {
                    signer: sig.signer_did.clone(),
                },
            };
            ...
            let signature = match Signature::from_slice(&sig_bytes) {
                Ok(s) => s,
                Err(e) => return VerificationResult::InvalidSignature {
                    signer: sig.signer_did.clone(),
...
mod tests {
    ...
    fn create_test_config(threshold: u32, num_signers: usize) -> (GovernanceConfig, Vec<SigningKey>) {
    ...
    fn sign_action(action: &GovernanceAction, signing_key: &SigningKey, did: &str) -> GovernanceSignature {
        let canonical = CanonicalGovernanceAction {
            proposal_id: action.proposal_id.clone(),
            action_type: action.action_type.clone(),
            proposed_at: action.proposed_at,
    ...
    fn governance_verifier_3_of_5() {
        ...
        assert!(matches!(result, VerificationResult::InsufficientSignatures { has: 0, needs: 3 }));
        
        // 2 signatures - still insufficient
        let mut action_2 = action.clone();
        action_2.signatures.push(sign_action(&action, &keys[0], "did:apodokimos:signer0"));
        action_2.signatures.push(sign_action(&action, &keys[1], "did:apodokimos:signer1"));
        let result = verifier.verify_action(&action_2);
        assert!(matches!(result, VerificationResult::InsufficientSignatures { has: 2, needs: 3 }));
        
        // 3 signatures - valid
        let mut action_3 = action.clone();
        action_3.signatures.push(sign_action(&action, &keys[0], "did:apodokimos:signer0"));
        action_3.signatures.push(sign_action(&action, &keys[1], "did:apodokimos:signer1"));
        action_3.signatures.push(sign_action(&action, &keys[2], "did:apodokimos:signer2"));
        let result = verifier.verify_action(&action_3);
        assert_eq!(result, VerificationResult::Valid);
        
        // 4 signatures - also valid
        let mut action_4 = action.clone();
        action_4.signatures.push(sign_action(&action, &keys[0], "did:apodokimos:signer0"));
        action_4.signatures.push(sign_action(&action, &keys[1], "did:apodokimos:signer1"));
        action_4.signatures.push(sign_action(&action, &keys[2], "did:apodokimos:signer2"));
        action_4.signatures.push(sign_action(&action, &keys[3], "did:apodokimos:signer3"));
    ...
    fn governance_unauthorized_signer() {
        ...
        bad_action.signatures.push(sign_action(&action, &keys[0], "did:apodokimos:signer0"));
        bad_action.signatures.push(sign_action(&action, &unauthorized_key, &unauthorized_signer.did));
        
        let result = verifier.verify_action(&bad_action);
        assert!(matches!(result, VerificationResult::UnauthorizedSigner { signer } if signer == "did:apodokimos:attacker"));
    }
    ...
    fn governance_invalid_signature() {
        ...
        let action = GovernanceAction {
            ...
            action_type: GovernanceActionType::Upgrade { new_version: "v0.3.0".to_string() },
            proposed_at: 3000,
            signatures: vec![],
        };
        ...
        let different_action = GovernanceAction {
            ...
            action_type: GovernanceActionType::Upgrade { new_version: "v0.4.0".to_string() },
            proposed_at: 4000,
            signatures: vec![],
        };
        ...
        let mut bad_action = action.clone();
        bad_action.signatures.push(sign_action(&different_action, &keys[0], "did:apodokimos:signer0"));
        bad_action.signatures.push(sign_action(&action, &keys[1], "did:apodokimos:signer1"));
        
        let result = verifier.verify_action(&bad_action);
        assert!(matches!(result, VerificationResult::InvalidSignature { signer, .. } if signer == "did:apodokimos:signer0"));
    }
    ...
    fn governance_duplicate_signature() {
        ...
        let result = verifier.verify_action(&bad_action);
        assert!(matches!(result, VerificationResult::InvalidSignature { signer, reason } if 
            signer == "did:apodokimos:signer0" && reason == "duplicate signature"));
    }
}
